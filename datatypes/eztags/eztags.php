...
class eZTags
{
    ...
    function store( $attribute )
    {
        ...
        $newTags = array();
        $tagsToLink = array();
        foreach ( array_keys( $this->IDArray ) as $index => $key )
        {
        ...
        {            
            $db->query( "UPDATE eztags_attribute_link SET priority = '{$index}' WHERE keyword_id = '{$key}' AND objectattribute_id='{$attributeID}' AND objectattribute_version='{$attributeVersion}' AND object_id='{$objectID}'" );
        }
    }
